...
export default function Admin() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {activeTab !== 'blog' && activeTab !== 'applications' && activeTab !== 'salaries' && activeTab !== 'payments' && (
      ...
      {activeTab === 'salaries' && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: 'Countries',     value: [...new Set(items.map((s: any) => s.country).filter(Boolean))].length, color: 'text-primary-600' },
            { label: 'Industries',    value: [...new Set(items.map((s: any) => s.industry).filter(Boolean))].length, color: 'text-blue-600' },
            { label: 'Cities',        value: [...new Set(items.map((s: any) => s.city).filter(Boolean))].length, color: 'text-green-600' },
          ].map(stat => (
            <div key={stat.label} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 text-center">
              <p className={`text-2xl font-bold ${stat.color}`}>{stat.value}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{stat.label}</p>
                    ...
                    <tr key={s.id as string} className="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                      ...
                      <td className="px-4 py-3 text-gray-600 dark:text-gray-300">
                        <div className="flex items-center gap-1">
                          <span>{(COUNTRY_FLAGS as Record<string, string>)[s.country] || '🌍'}</span>
